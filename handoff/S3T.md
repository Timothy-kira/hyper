...
**v3 的前 1400 步**（21 分钟），两个比值都是模型误差 ÷ 平凡填充的误差，低于 1 就说明比平凡填充好：

| 指标 | 数值 |
| --- | --- |
| band/interp（光谱补全 vs 按波长做线性插值） | **0.45** |
| spat/mean（空间补全 vs 可见区域的均值） | **0.59** |
| grey（被遮区域预测成整块灰的比例） | 0.01 |
| 速度 | 114 crops/s，是 v2 的 3.6 倍 |

v2 报的 0.60 里有泄漏的功劳；v3 没有泄漏，还是到了 0.45。
